...
class MetalRollManager:  # Менеджер работы с бд
    ...
    @staticmethod
    async def filter_roll(f: FilterAll):
        """
    ...
    @staticmethod
    async def statistics(start, end):
        """
        ...
        stats = await MetalRollManager.day_stats(start, end)
    ...
    async def day_stats(start, end):
        ...
        async with await get_async_session() as session:
            for i in [StatisticsDay.countMetalRoll, StatisticsDay.sumWeightMetalRoll]:
                ...
                min_ = (await session.execute(query)).one()
                query = help_selects(start, end, func.max(i), i)
                max_ = (await session.execute(query)).one()
