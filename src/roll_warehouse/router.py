...
async def get_rolls_by_filters(
        id_start: int = Query(default=None, gt=0, alias='Стартовый индекс (больше 0)'),
        id_end: int = Query(default=None, gt=0, alias='Конечный индекс (больше 0)'),
        length_start: int = Query(default=None, gt=0, alias='Начальная длинна (больше 0)'),
        length_end: int = Query(default=None, gt=0, alias='Конечная длинна (больше 0)'),
        weight_start: int = Query(default=None, gt=0, alias='Начальный вес (больше 0)'),
        weight_end: int = Query(default=None, gt=0, alias='Конечный вес (больше 0)'),
        data_add_start: datetime = Query(default=None, alias='Начальное время UTC'),
        data_add_end: datetime = Query(default=None, alias='Конечное время UTC'),
        data_del_start: datetime = Query(default=None, alias='Время удаления старт UTC'),
        data_del_end: datetime = Query(default=None, alias='Время удаления конец UTC')
    ...
    try:
        ...
        rolls = await MetalRollManager.filter_roll(f)
        return rolls
...
async def add_datab(
    ...
    try:
        roll_orm = await MetalRollManager.add_t(roll)
        return roll_orm
...
async def delete(
    ...
    try:
        roll_orm = await MetalRollManager.delete(id_roll)
        return roll_orm
...
) -> dict:  # получаю всю статистику нужную по заданию
    try:
        ...
        statistics = await MetalRollManager.statistics(date_start, date_end)
        return statistics
    except ConnectionDoesNotExistError as e:
        print(e)
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE)
